...
|`--debug-agent`|显式在回环地址启用 pprof，例如 `127.0.0.1:6060`；默认关闭。|
|`--no-agent`|用于兼容旧配置的废弃选项；即使同时指定 `--debug-agent` 也保持 pprof 关闭。|
...
|`--secret-key=value`|对象存储的 Secret Key，也可通过环境变量 `SECRET_KEY` 设置。查看[如何设置对象存储](../reference/how_to_set_up_object_storage.md#aksk)以了解更多。|
|`--session-token=value`|对象存储的临时访问凭证（Session Token），查看[如何设置对象存储](../reference/how_to_set_up_object_storage.md#session-token)以了解更多。|
|`--tier=value` <VersionAdd>1.4</VersionAdd>|分层存储 ID（0-3），与 `--storage-class` 配合使用，用于配置指定 tier 的存储类型。|
|`--storage-class=value` <VersionAdd>1.1</VersionAdd>|分层存储对应的对象存储类型（例如 `STANDARD_IA`、`INTELLIGENT_TIERING`、`GLACIER_IR`），可与 `--tier` 同时使用。|
...
|`--min-client-version value` <VersionAdd>1.1</VersionAdd>|允许连接的最小客户端版本，只能提高，不能降低|
...
|`--changelog` <VersionAdd>1.4</VersionAdd>|启用[元数据 changelog](../administration/changelog.md)（默认值：false）|
|`--changelog-max-age` <VersionAdd>1.4</VersionAdd>|changelog 条目的最长保留时间，例如 `2h` 或 `30m`；`0` 表示不按时间清理|
|`--changelog-max-lines` <VersionAdd>1.4</VersionAdd>|最多保留的 changelog 条目数量；`0` 表示不限制|
...
跟踪显示文件系统元数据操作的 changelog。用法和保留策略详见[「元数据 Changelog」](../administration/changelog.md)。使用前需要先启用 changelog 功能：
...
|-|-|
|`META-URL`|用于元数据存储的数据库 URL，详情查看[「JuiceFS 支持的元数据引擎」](../reference/how_to_set_up_metadata_engine.md)。|
|`--from=0`|从指定元数据版本开始显示 changelog。`0` 表示从最新版本开始跟踪。|
...
|`--flush-timeout value`|等待 `--flush` 的最长时间；0 表示不设超时（默认值：10m）|

### `juicefs durability` {#durability}

等待 writeback 模式暂存的数据持久化到对象存储，或查看当前写回状态。该命令
只能由 root 或挂载进程的所属用户执行。
...
|项 | 说明|
|-|-|
|`--status`|仅查看当前写回持久化状态，不等待|
|`--timeout value`|等待屏障的最长时间；0 表示不设超时（默认值：10m）|
|`--json`|以 JSON 输出结果|
...
|`--head-dir`|标准的 S3 协议是没有「目录」这一概念的，因此默认情况下，为了符合 S3 协议，该选项是关闭的，此时对目录进行 HEAD 请求，会返回 404，除非该是显式 PUT 创建的（用类似 `aws s3api put-object --key /path/dirname/` 的方式）。<br/><br/>在 `--head-dir` 未开启的情况下，S3 Gateway 会使用目录的 atime 作为目录是否显式创建的判断标准：如果 atime 为 0，则视为显式创建，可以进行 HEAD 请求；如果 atime 大于 0，则视为隐式创建，对其发起 HEAD 请求会返回 404。<br/><br/>在某些场景下，如果希望能够对 所有目录发起 HEAD 请求，可以启用 `--head-dir` 参数，让文件系统内的所有目录，都可以当做普通的 S3 对象来查询。如果要使用该功能，建议升级到 5.3.8 或者更新版本，来避免一些已知问题。|
|`--hide-dir-object`|通过 S3 Gateway 在文件系统（尚不存在的）深层目录显式创建文件，目录层级因此被动创建出来，这种目录属于隐式创建目录。<br/><br/>`--hide-dir-object` 这个参数的作用是隐藏通过 PUT 请求隐式创建的目录。该选项默认关闭，在此情形下，系统会通过 atime 决定目录是否展示：如果 atime 为 0，则目录会出现在 LIST 请求中；如果 atime 大于 0，则不会显示。<br/><br/>如果开启该选项，则彻底隐藏隐式创建的目录，只有用类似 `aws s3api put-object --key /path/dirname/` 的方式显式创建目录（atime 为 0），才能够被 LIST 请求展示出来。|
...
文件中的每一行，或者是一个路径（预热整个文件），或者是一个路径加上方括号包裹的字节范围列表。
范围为 `start-end` 形式，多个范围之间用 `;` 分隔，其中 `start` 包含在内、`end` 不包含在内，
因此 `[0-4096]` 表示开头的 4096 字节。由于方括号本身起到界定作用，含空格的路径无需引号。
如果某一行的方括号内容不合法，该行会被报告并跳过，而不会被当作普通路径处理。

字节范围决定了操作哪些数据块，而块是缓存的最小单位，因此一个范围至少会覆盖一个完整的块
（由 `--block-size` 决定，默认 4 MiB）。使用 `--evict` 时尤其需要注意，范围涉及的块会被整块清除；
`--check` 的统计结果同样以块为单位。
...
|项 | 说明|
|-|-|
|`--file=value, -f value`|指定一个包含一组路径的文件（每一行为 `path` 或 `path [start-end;...]`）。|
...
|`--exclude=PATTERN`|排除匹配 `PATTERN` 的 Key。参考[「模式匹配」](../guide/sync.md#pattern-matching)文档了解如何使用。|
|`--include=PATTERN`|不排除匹配 `PATTERN` 的 Key，需要与 `--exclude` 选项配合使用。参考[「模式匹配」](../guide/sync.md#pattern-matching)文档了解如何使用。|
|`--match-full-path` <VersionAdd>1.2</VersionAdd>|使用「完整路径过滤模式」，默认为 false。参考[「完整路径过滤模式」](../guide/sync.md#full-path-filtering-mode)文档了解如何使用。|
...
完整使用指南（含参考服务端实现）请参阅[全局流量控制](../guide/sync.md#global-traffic-control)。
...
| 项 | 说明 |
|-|-|
|`--tier value`|tier ID（0-3）。`0`为默认层，`tier set` 时必填。|
|`--recursive, -r`|递归处理目标目录下的所有文件和子目录（默认：false）。|
|`--force, -f`|强制将对象重写到 tier 当前映射的存储类型，即使 tier id 未改变（默认：false）。适用于 `--storage-class` 变更后需要迁移已有对象的场景。|
