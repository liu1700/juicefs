...
通过 `--access-log` 或 SDK 参数配置的访问日志，在 POSIX 系统上默认以仅属主可访问的权限（`0600`）创建。JuiceFS 会拒绝符号链接、非普通文件、属于其他用户的文件以及具有多个硬链接的文件；配置了不安全的日志目标时，客户端将拒绝启动。已有的 owner-only 权限会保留，旧版本留下的组或全局可访问日志会收紧为 `0600`。轮转始终通过已打开的文件描述符继续写入，失败会明确报告且不会截断日志。
