...
:::note 路径中包含特殊字符
`juicefs sync` 会将数据源和目标端的路径当作 URL 来解析，因此如果路径中含有 `#` 等在 URL 中具有特殊含义的字符，需要先进行转义（URL 编码），否则路径会被截断或解析错误。例如，`#` 需要写成 `%23`，路径 `a/b#c/d` 应当写成 `a/b%23c/d`。
:::
...
`file://` 对象存储 API 不会跟随目标位于配置根目录之外的符号链接。链接本身仍会保留并可通过 `readlink` 检查，但通过对象存储 API 发起的 `Head`、`Get`、权限修改和时间戳修改会拒绝越界访问。请仅对可信输入使用 `--links`，因为直接操作目标目录的系统工具仍可能跟随复制后的链接。
