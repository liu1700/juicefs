...
| `juicefs.access-log`    |               | Access log path, e.g. `/tmp/juicefs-${user.name}.access.log`. The file is owner-only on POSIX systems, so applications running as different users should use different paths. The log rotates automatically to keep at most 7 files. |
