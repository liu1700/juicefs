...
|`--debug-agent`|Explicitly enable pprof on a loopback address, for example `127.0.0.1:6060`. Disabled by default.|
|`--no-agent`|Deprecated compatibility flag. Keep pprof disabled even if `--debug-agent` is also supplied.|
...
|`--secret-key value`|Secret Key for object storage (can also be set via the environment variable `SECRET_KEY`), see [How to Set Up Object Storage](../reference/how_to_set_up_object_storage.md#aksk) for more.|
|`--session-token=value`|session token for object storage, see [How to Set Up Object Storage](../reference/how_to_set_up_object_storage.md#session-token) for more.|
|`--tier=value` <VersionAdd>1.4</VersionAdd>|Storage tier ID (0–3), used together with `--storage-class` to configure the storage class for a specific tier. ID `0` is the default tier.|
|`--storage-class=value` <VersionAdd>1.1</VersionAdd>|Storage class for the specified tier (for example, `STANDARD_IA`, `INTELLIGENT_TIERING`, and `GLACIER_IR`), including tier `0` for the default tier. Used together with `--tier`.|
...
|`--min-client-version value` <VersionAdd>1.1</VersionAdd> |minimum client version allowed to connect; can only be increased|
...
|Items|Description|
|-|-|
|`META-URL`|URL of the metadata engine database. See [JuiceFS supported metadata engines](../reference/how_to_set_up_metadata_engine.md) for details.|
|`--from=0`|show changelog starting from the specified metadata version. `0` means tailing from the latest version.|
...
|`--flush-timeout value`|maximum time to wait for `--flush`; 0 means no timeout (default: 10m)|

### `juicefs durability` {#durability}

Wait for data staged by writeback mode to become durable in object storage, or
inspect the current writeback status. The command must be run as root or as the
user that owns the mount process.

#### Synopsis

```shell
juicefs durability [command options] MOUNTPOINT

juicefs durability --timeout 10m /mnt/jfs
juicefs durability --status --json /mnt/jfs
```
...
|Items|Description|
|-|-|
|`--status`|show current writeback durability status without waiting|
|`--timeout value`|maximum time to wait for the barrier; 0 means no timeout (default: 10m)|
...
|`--head-dir`|The notion of "directory" does not exist in the standard S3 protocol, so this option is disabled by default to remain protocol-compliant. When disabled, HEAD requests to directories return 404 unless the directory was explicitly created by a PUT request such as `aws s3api put-object --key /path/dirname/`. <br/><br/>When `--head-dir` is disabled, the S3 Gateway uses a directory's atime to determine whether it was explicitly created: atime 0 means explicitly created and allowed for HEAD requests; atime greater than 0 means implicitly created and HEAD requests return 404. <br/><br/>If you need HEAD requests to succeed for all directories, enable `--head-dir` to treat directories as ordinary S3 objects. It is recommended to upgrade to 5.3.8 or later when using this option to avoid known issues.|
|`--hide-dir-object`|When S3 Gateway implicitly creates a deep directory structure because a file is PUT into a path under directories that do not yet exist, those directories are considered implicitly created. <br/><br/>The `--hide-dir-object` option hides those implicitly created directories. This option is disabled by default, in which case the gateway uses atime to decide whether a directory can be listed: atime 0 directories appear in LIST results; atime greater than 0 directories do not. <br/><br/>When enabled, implicitly created directories are hidden completely. Only directories explicitly created by a PUT request such as `aws s3api put-object --key /path/dirname/` (atime 0) will be shown in LIST responses.|
...
# Warm up only some byte ranges of a file
echo '/jfs/f1
/jfs/f2 [0-4096]
/jfs/f3 [0-4096;1048576-1052672]' > /tmp/filelist.txt
juicefs warmup -f /tmp/filelist.txt
```

Each line of the file is either a path, which warms up the whole file, or a path
followed by a bracketed list of byte ranges. Ranges are `start-end` pairs
separated by `;`, where `start` is inclusive and `end` is exclusive, so `[0-4096]`
covers the first 4096 bytes. Because the brackets delimit the list, paths
containing spaces need no quoting. A line whose bracketed list is malformed is
reported and skipped, rather than being treated as a path.

Ranges select which blocks to work on, and a block is the smallest unit of the
cache, so a range always covers at least one whole block (`--block-size`, 4 MiB
by default). Keep this in mind with `--evict`, which discards every block the
ranges touch, and with `--check`, which reports whole blocks as well.

#### Options

|Items|Description|
|-|-|
|`--file=path, -f path`|file containing a list of paths (each line is `path` or `path [start-end;...]`)|
...
|`--exclude=PATTERN`|Exclude keys matching `PATTERN`. Refer to the ["Pattern matching"](../guide/sync.md#pattern-matching) document to learn how to use it.|
|`--include=PATTERN`|Include keys matching `PATTERN`, need to be used with `--exclude`. Refer to the ["Pattern matching"](../guide/sync.md#pattern-matching) document to learn how to use it.|
|`--match-full-path` <VersionAdd>1.2</VersionAdd>|Use "Full path filtering mode", default is false. Refer to the ["Full path filtering"](../guide/sync.md#full-path-filtering-mode) document to learn how to use it.|
...
|`--bwlimit=0`|Limits bandwidth in Mbps. The default is 0, which means unlimited. When used together with `--traffic-control-url`, it acts as a per-process fallback when the global traffic control service is unavailable. Fallback waits already in progress remain under `--bwlimit`, and subsequent waits return to the global limit after recovery. Set it to a smaller value than the global cap to make the fallback meaningful.|
|`--traffic-control-url value` <VersionAdd>1.4</VersionAdd>|URL of the global traffic control service for coordinating bandwidth across multiple sync instances. The service must expose an HTTP POST endpoint with request body `{"bytes": <requested-bytes>}` and response body `{"granted": <granted-bytes>, "expired": <expire-time-ms>}`. It is tried before `--bwlimit`; if unreachable, current limiter checks fall back to `--bwlimit` (or no limit if unset), and subsequent checks use the global limit again after recovery.|
...
Refer to [Global traffic control](../guide/sync.md#global-traffic-control) for a complete usage guide including a reference server implementation.
...
`juicefs tier` manages storage tiers. For detailed information, refer to [Tiered Storage](../guide/tiered-storage.md).
...
| Item | Description |
|-|-|
|`--tier value`|Tier ID (0-3). `0` is the default tier (reserved). `1` to `3` are user‑configurable. Required for `tier set`.|
|`--recursive, -r`|Recursively process all files and subdirectories under the target directory (default: false).|
|`--force, -f`|Force rewriting objects to the tier's current storage class, even if the tier ID remains unchanged (default: false). Use after changing `--storage-class` to migrate existing objects.|
