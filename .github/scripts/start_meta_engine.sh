...
install_tidb(){
    ...
    if lsof -i:4000 -sTCP:LISTEN && pgrep pd-server && timeout 10 mysql --connect-timeout=5 -h127.0.0.1 -P4000 -uroot -e "select version();"; then
        echo "TiDB is already running and healthy"
        return 0
    fi
    # Only an unhealthy playground is torn down: a half-started cluster keeps
    # the ports busy and makes the next attempt fail too.
    stop_tiup_playground
    user=$(whoami)
    echo user is $user
    if [[ "$user" == "root" ]]; then
